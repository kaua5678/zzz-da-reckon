...
- **Boss 预设**：`scripts/fetch-nanoka-bosses.mjs` + `scripts/import-nanoka-bosses.mjs` → `public/static/boss-presets.json`（危局强袭战 19 个 Boss（含新补录 恶名·冥宁芙 / 「亵渎者」 / 叛律孤歌·薇斯珀 / 猎血清道夫）× 各期血量/失衡/防御/异常系数/三张抗性表 + 怪物本体失衡倍率/失衡时间 + 用户确认收录的老防卫战 Boss 彷徨猎手），属性配置页 `BossSelectCard.vue` 一键填充。
