  ...
  <div class="page-root">
    ...
    <n-card size="small" class="block" title="待确认口径（按原表保留，不参与纵向聚合强约束）">
      <ul class="notes">
        ...
        <li>倍率行融合：catalog 把一套招式拆成多行/多段时按配置加总为一个评估单元（前缀项/耗能只计一次）。已配置：星见雅「飞雪 #1+#2=斩击、#3+#4=追击」（融合后 = nanoka 官方倍率，代入公式 ≈1.001）、连携「春临 #1~#3」三段合一（≈1.000，逐段会有 ~0.38 假象）。</li>
        <li>仪玄强特（定点整链记录，FIXED_RECORD_UNITS）：墨痕化形按交互变体记录 6 招 = 单E/2连/3连 × 弹刀与否——#1=1371009（40 闪能）、#2=1371024（弹刀赠送，免费）、#3=1371023（跟随#1，免费）、#4=1371025（20 闪能）；单E/2连 = 40 闪能、3连 = 60 闪能、弹刀追加免费 #2（回 10 闪能）。每招 = 完整一次链：t 与各列数值求和、耗能整链一次计；原「#1+#2=斩击 / #3+#4=追击」融合评估得 ~0.46~2.56 假象（追击头行无耗能标注），已弃用。凝云术（2s 通道共 40 即 20/s）/墨烬影消（20）保持逐行评估；特调由各变体比值如实呈现（单E 伤害 ~0.82、3连·弹刀 喧响/积蓄 ≈0.99、全变体秽盾 ≈1.000）。</li>
        <li>普罗米娅强特（定点整链记录）：处刑式·坠霜/重霜实为消耗 40 能量（封喉霜径 1541009 / 匿影）后接的免费招，记录为强特公式 + 40 能量奖励（否则按特殊技公式 e=0 得 2.2~3.1 倍虚高比值、污染纵向积蓄系数）；重霜 #1+#2 合一（#2 子段 24.2%、t=0）。评估：重霜 伤害 ≈1.023、坠霜 ≈0.922、秽盾 ≈1.000；1541 纵向积蓄由污染值 125.1% 回落到干净样本 85.0%。封喉霜径（40 能量消耗者）保留原行（t=0 仅展示专属资源）。</li>
        ...
        <li>普攻回能数据缺口：以下招式回能显式录 0，按「0% + 回能录入为0」展示且不参与纵向聚合——柚叶「狸之帐」、克拉蕾「血锻四式 ×4 / 锻星 ×3」、洛克茜「旋风锤」，真实值待第二源复核（gachabase 爬取脚本已备好）。爱丽丝「星芒圆舞曲」三段经 gachabase 双源确认为官方 0（非缺口），同样以 0% 展示。</li>
        <li>席德(1461)错位攻击数据已修复（scripts/patch-misplaced-attack-data.mjs，用户 2026-08 观察）：attack_data 实为 [钢能, 回能, 喧响] 三通道，数值策划把回能/喧响填进了 attack_data 的 1/2 位、正列反为 0（如磁陨轮舞 [3.2084, 0.73, 8.03]，8.03/0.292s=27.5/s 精确命中喧响基准、0.73/0.292=2.5/s 与霜蕊轮舞 #3/#4 正列同速率）。7 招的 energy_recovery/decibel_recovery 已回填（回能速率 2.5/s → 纵向回能 69.5%、喧响 27.5t → 100%），attack_data_1/2 错位行已移除，attack_data_0（钢能）保留。席德「霜蕊轮舞 #1/#2」随之脱离回能缺口清单。</li>
