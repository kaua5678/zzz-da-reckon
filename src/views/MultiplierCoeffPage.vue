<template>
  <div class="page-root">
    ...
    <n-card size="small" class="block" title="待确认口径（按原表保留，不参与纵向聚合强约束）">
      <ul class="notes">
        <li>轻/重招架：官方口径 92.4+130t / 89.1+130t（2026-08 修正，旧表偏小 ~10%）；catalog 实录隐含截距 ~95.4+130t，比官方口径高 ~1.4%/~2.6%，残差待确认（两段实际仅差 0.3，官方口径的 3.3 差值未在数据中复现）。</li>
        <li>快速支援：伤害/失衡实测 ≈2× 标准、喧响 ≈55%，且新旧角色分层减半——疑似部分快支的秽盾基准实为 200t（时间口径差一倍所致）；喧响 27.5/s 校准仅对喧响未特调的角色成立，见上方校准清单。</li>
        <li>仪玄强特（墨痕化形/凝云术/墨烬影消）：强特后可跟免费招式、是否与本体合并计算待定，特调严重——已按用户口径暂不展示、不参与评估；其凝云术/墨痕 nanoka 记「40 点」与「20/秒」的说法不一致，耗能口径待确认。</li>
        <li>般岳「支援突击：冲霄」：金身格挡后招式，已按闪避反击公式定点改判（般岳锚点=「昂霄」，比值 ≈1.000）；其录制 t=2.667s 疑含金身持盾时间（喧响/积蓄反推有效 t≈1.167s），伤害/失衡另叠 ~0.93 特调。</li>
        <li>支援突击「#N」分段按同套合并（前缀项只计一次；苍角实证三列 ≈1.000）；真斗「孤影·断獠」合并后伤害 ≈1.02 但失衡/喧响显著偏离（两段喧响各自精确等于闪避反击 27.5t），分段真实构成待确认。</li>
        <li>强化特殊技：整体公式成立（伤害中位数 ~101%），但逐角色偏离大，属设计空间；闪能 ×1.2 规则已经真斗（耗 80 闪能）五列 ≈1.000 验证。</li>
