...
describe('R15-c 跳①：叶瞬光非轴 —— 白名单满易伤 / 非白名单回落覆盖率（成对精确值）', () => {
  it('同一队同一轮：1431013 → stunMult 精确 1.5；1431016 → 精确 1.15（覆盖率折扣值）', async () => {
    ...
    // 2026-09-19 R37-J5 再动：①动态合轴（队友前台按溢出量被吸收，池/账本变）⇒ 17.040；④叶瞬光强特时间单源（estimate 与
    // buildExecutions 同一 cycle）⇒ 16.618。该队默认口径外层是 0 ↔ 1.02 的 2-循环，停点 = 1.02 窗成员（⑥′ 环内选点：时间差 < 2s 取最后一轮）。
    // 2026-09-19 吸收上限 40%（用户 v3）：1 窗成员多出 4.4s 截断 ⇒ 16.571；⑥″ 零窗成员不参选，落点仍是 1.02 窗成员。
    /**
     * ⚠ 2026-09-20 基线 16.571 → **16**：叶瞬光**终局整数化**（`@fact agent:1431/终局整数化`，
     * 用户口径「余数剑势本来就该留着不打」）——明心境轮数在终局 floor 一次，段数 = 轮数 × 每轮段数
     * ⇒ 灭#1 = 8 轮 × 2 = **16**（不再有 8.29 轮这种小数轮）。
     * 迭代期仍实数（防「平A↑→剑势↑→轮数+1整轮」正反馈环），语义由
     * `yeshuguang.test.ts#轮数实数化` 钉；本条探针读的是终局物化行 ⇒ 按整数解。
     * ⚠ 同时新增的**环成员可行性闸门**（见 `useResourceCalc#runOuterLoop`）让该队的停点从
     * 「0↔1.02 的 2-循环里那个装不下的成员」变为**装得下的成员** ⇒ 轮数 8（此前 8.29 是含截断的）。
     */
    expect(full[0].count).toBeCloseTo(16, 3)
