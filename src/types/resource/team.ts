...
export interface ConvergenceReport {
  ...
  /**
   * 外层不动点判 `cycle` 时的停点选法（R37-J5 ⑥，2026-09-19，用户裁决「治本」；⑥′ 同日校准）：环内成员按
   * ① **失衡自洽度** |输出 next − 输入 stunCount|（容差 0.05）② **时间自洽度**（|预算 − Σ物化净占用| + 装配截断秒数，
   * 差 ≤ 2s 视为同级）③ 最后一轮 的顺序取点，不再取「碰巧最后算的那轮」；true = 选中的不是最后一轮。
   * 2-循环两成员的失衡自洽度恒相等 ⇒ 由时间面决定「循环算到最后只溢出一点或不溢出」的落点；长环先取离失衡不动点最近者
   * （首版只看时间面时被「反馈线程滞后」「冷启动 0 窗」两类成员骗过，判据来历见 useResourceCalc#pickCanonical 注释）。
   */
  outerCyclePickedEarlier?: boolean
  /**
