...
export interface ConvergenceReport {
  ...
  /**
   * 外层不动点判 `cycle` 时的停点选法（R37-J5 ⑥，2026-09-19，用户裁决「治本」）：环内成员按**时间自洽度**
   * （|预算 − Σ物化净占用| + 装配截断秒数）取最小者，不再取「碰巧最后算的那轮」；true = 选中的不是最后一轮。
   * 环内成员在 stun 次数上本就等价（互为映射），时间面最自洽的那个才是「循环算到最后只溢出一点或不溢出」的落点。
   */
