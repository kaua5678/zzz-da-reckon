...
describe('菲欧妮（1641）⚠️3.3 测试服临时录入', () => {
  ...
  it('面板差分：核心异常精通+40、脆弱异常暴击率/伤进面板（引擎 EV 乘区字段）；影画2 积蓄效率 ×覆盖率', async () => {
    ...
    // 异常掌控取局外面板值（词条默认无掌控加成 → 基础 118 < 145 → 率 = 30）
  ...
  it('面板差分：队伍异常角色数滑块驱动暴伤档位（2=25 / 3=40，需额外能力触发——同阵营或异常队友）', async () => {
    // 1171 柏妮思 = 异常角色 → 额外能力触发；队伍异常数档位由滑块表达（菲欧妮+队友 = 2 档）
    const { config, computePanelPhases } = await setup(['1641', '1171', ''])
    config.setMechanicSetting('phoenix.teamAnomalyCount', 2)
    const p2 = computePanelPhases(0, config, useCatalogStore())!.inCombat as any
    expect(p2.anomalyCritDmg).toBeCloseTo(25, 5)
    config.setMechanicSetting('phoenix.teamAnomalyCount', 3)
    const p3 = computePanelPhases(0, config, useCatalogStore())!.inCombat as any
    expect(p3.anomalyCritDmg).toBeCloseTo(40, 5)
  })
  ...
  it('模块注册与滑块：4 个滑块齐全', () => {
    ...
    expect((phoenixMechanic.settings ?? []).map(s => s.id).sort()).toEqual([
      'phoenix.c2IncinerationCoverage', 'phoenix.chargedAttackCount', 'phoenix.releaseCoverage', 'phoenix.teamAnomalyCount',
    ])
  })
})
