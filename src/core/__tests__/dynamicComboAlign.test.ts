...
describe('动态合轴 · 操作角色不动、队友前台按溢出量被合轴吸收', () => {
  ...
  it('①b 缺省上限 0.4（用户 v3）：溢出 > 容量的 1431 簇——队友终态前台恰有 40% 被并行、剩余溢出回到封顶/装配截断如实上报，强度介于无吸收与全额吸收之间', async () => {
    ...
    for (const id of ['auto-1431-1481-1491', 'auto-1431-1481-1341'] as const) {
      ...
      for (const s of capped.slots) {
        if (s === op) continue
        // 上限绑定：每名队友终态账本前台里被并行的份额 == 上限（容差 1%）
        expect(s.dynamic / Math.max(1e-9, s.necessary), `${id} ${s.agentId} 被并行份额`).toBeCloseTo(DEFAULT_COMBO_ALIGN_ABSORB_RATIO, 2)
      }
      expect(op.frontline, `${id} 操作角色不再独占战斗时间`).toBeLessThan(capped.budget - 30)
      expect(capped.cut, `${id} 超过上限的溢出如实截断`).toBeGreaterThan(TIME_BUDGET_TOLERANCE_SECONDS)
      expect(capped.cut, `${id} 但少于完全不吸收`).toBeLessThan(none.cut)
