/**
 * ⚠️ 无调用点（2026-09-12 评审复核，规则 16① 口径体检发现）：本表是 08-20 的异常结算设计快照，
 * 全仓（src/scripts/docs）**零引用**——表中声明的"基础区/结算区/异常增伤区"口径，**活实现**在
 * `core/anomalyPool.ts` / `core/anomalyPool/helpers.ts` 与 `core/damage.ts` / `core/buff.ts`
 * 的结算区通道里（那里才是单一事实源，改口径请去那边）。
 * 保留仅作历史参考（含若干"用户核对版"结论）；**最终处置（迁入 docs / 删除）待用户裁决**，
 * 在此之前勿把它当作现状口径引用。
 *
 * ———— 以下为原表说明（截至 08-20 的设计快照）————
 *
