...
6. **跨角色/队伍级机制走 `applyTeamConfig` 钩子**（三阶段 build/converge/postRound，见 `docs/ENGINE_PIPELINE_GUIDE.md` §2），禁止往 `useResourceCalc` 新增**角色判定**分支（机器护栏：`check-guards` 角色判定棘轮冻结存量基线，新增即红；存量清零后下调基线）。**度量口径 = AST 三形态**（`agentId` / `.id` 四位数字 / `teammateBuffId`，按行去重；单源 `scripts/lib/agent-identity-lines.mjs`，分类报告 `node scripts/report-agent-identity.mjs --md`）——2026-09-17 round 19 换尺：旧正则只认 `agentId ===`，实测**漏计 27 行**同义判定（`agent?.id === '1581'` / `agent.teammateBuffId === 'remielle'`），新写的这两种形态**永远不会被拦**；换尺属规则 17② 的「口径纠正」（frozen 15→42，不是退步）。面板阶段的覆盖率滑块直接读 `AgentPanelInput.settings`，不要经 panel 字段走私（曾致般岳滑块静默失效）。**棘轮有 burn-down 契约**（`check-guards` 的 `RATCHET_BURNDOWN`：每条登记 frozen/target/due/plan），`zc status` 会点名「已到期且零进展」的棘轮——棘轮防变差，burn-down 防「冻结 = 永久豁免」，降了基线就同步下调 `frozen` 与进度。
...
17. **★ 永远选长期（用户裁决 2026-09-17，本条优先于一切"少改/先凑合/怕数字难看"的冲动）**
    本仓库**仍在开发中**，未上线、无外部用户 ⇒ **重构规模本身不是否决理由**。**有优选优，不要思维固化。**
    ① **不要在"短期 vs 长期"之间提问**——这是**已被裁决的元问题**，答案恒为长期。
       遇到它直接按长期做，**不要再回来问用户**；把它当可自主推进项写进账本即可。
    ② **度量口径纠正不适用「棘轮只减不增」**：棘轮防的是**代码变差**，不是防**尺子变准**。
       发现度量漏计形态（如 `agentId ===` 正则漏掉 `.id ===` / `teammateBuffId` 形态）时，
       **自主把 frozen 调到真实值**并在沿革里写「这是口径纠正不是退步」
       （2026-09-12 先例：frozen 8 → 78，注释原文「78 才是编排层真实的 agentId 特判存量」）。
       ⚠ 唯一纪律 = **换尺与改代码不得混批**（三份规划全票一致的禁忌）：混批会让 delta 无法归因。
    ③ **不许为保住好看的数字而维持错误的度量**——那是"解决提问题的人"。规则 10 已明令：
       基线是测量工具，不是开发否决权。
    ④ **顺序在两条路都必做时不是决策点**：若 A、B 最终都要做且无依赖，按自然顺序做掉，
       不要把"先做哪个"当成需要用户裁决的问题。**这是 agent 自主交接的范畴。**
    ⑤ **"大面积重构"不写进风险栏**——风险栏写的是"会错/不可逆/需用户口径"，不是"改动行数多"。
       改动大 ⇒ 切批 + 逐批可机检，而不是不做。
    ⑥ **口径纠正先分类，再定计量单位**：区分业务选择、纯身份定义、待核查项；比较表达式数、
       去重行数、迁移批次数分别报告。纯身份定义可做 DRY 收敛；**`find/filter` 回调里的身份比较
       仍在选择业务数据，不能仅因有 `return` 就豁免**。未知项显式列出并追踪消费者，再定 frozen。
