...
export const DEBT_REGISTRY = {
  ...
  // 仪玄实测行级 5628 vs 聚合 1702；约 50 个模块有 decibelRecovery:0 硬编码。修复 = 喧响账本改
  // 行级求和（与 damagePool 同构），能量随同一次账本重构迁移；全库喧响→终结次数需再基线。
  'src/core/resource/helpers.ts:喧响收入行级化': { since: '2026-09-07', due: '喧响账本改行级求和（Σ decibelRecovery×count，与 damagePool 同构），全库喧响→终结次数再基线' },
  'src/core/resource/helpers.ts:能量收入行级化': { since: '2026-09-07', due: '随喧响行级化同一次账本重构迁移（能量收入改行级求和）' },
 ...
 */
// @fact engine:guards/自指豁免 口径: 扫描器自身含被扫模式的字面量属自指、不计违规（fetch-stub 用 GUARD_SYSTEM_FILES，debt 用本清单，事实扫描用占位符跳过） | 据 实测@2026-09-01·复核@2026-09-04 | 验 src/scripts/__tests__/zc.test.ts | 锚 scripts/check-guards.mjs#DEBT_SCAN_SELF_REFERENTIAL | 信 确认
