...
| 把「状态机没建模」当成「次数驱动效果也不能建」 | 消费型状态效果（退出时+X）与「入场触发」招式整条丢进未建模清单，收益凭空消失 | 1641 菲欧妮第一版：[消亡]→连携+30%积蓄、[终结技：入场]（1545.8%）全列未建模——用户抓出：消亡由终结进入⇒**进入次数=终结次数**；每次消亡被下一次连携消费⇒**加成次数=min(终结,连携)**；终结技原文有「招式发动后，可点按发动[终结技：入场]」⇒**每次终结后点按一次，计数=终结次数**（仪玄影画6「赠送次数=大招次数」同款用户口径） | ①状态由什么动作**进入**→进入次数=该动作执行行次数（state.ultimateCount 等）；②消费型加成=比例乘区（行级 anomalyBuildUp ×(1+30%×占比)，南宫羽×1.35/妮可C1 先例）；③「切换/快支入场时发动」类先重读有无「招式发动后可点按」句——有=每次大一次的稳定计数，没有才按切场时序留未建模 |
