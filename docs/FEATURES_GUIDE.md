...
- **待确认口径（页面底部有同款清单）**：个别减半/离群的快速支援记录（安东、卢西娅等，按标准版评估由偏差清单呈现）。已定案：**快速支援两版口径**——标准版 = 伤害 200t（原表 100t 系笔误）+ 原表其余列；翻倍版 = 录制时间只有一半（秽盾基准 50t），评估按 `MOVE_TYPE_TIME_SCALE` ×2 还原、伤害 100t、其余列同标准版；分版判据 = 喧响速率 ≥40/s。**倍率行融合 `MOVE_FUSION_GROUPS`**：星见雅「飞雪 #1+#2=斩击 / #3+#4=追击」（融合后=nanoka 官方倍率，代入公式 ≈1.001）、连携「春临 #1~#3」三段合一（≈1.000）、仪玄「墨痕化形 #1+#2 / #3+#4」——前缀项/耗能只计一次。仪玄强特恢复展示（耗能与 catalog 一致：凝云术 2s 通道共 40、墨痕化形 #1=40/#4=20、墨烬影消=20），特调严重由偏差如实呈现。另有定案：轻/重招架标准常数 = 数据校准值 **95.511+130t / 95.178+130t**；般岳「冲霄」= 闪避反击公式 + 时间 −1.5s；真斗「孤影·断獠」= 改判闪避反击，直伤记「—」；露西两段强特各耗 60 各自成式；妮可 #1 蓄力 20 能量/s、#2/#3 炮击每次耗 60 并留能量场、#4 能量场——明细仅供参考。
