...
- **待确认口径（页面底部有同款清单）**：快速支援伤害/失衡 ≈2×、喧响 ≈55% 且新旧分层——已确认存在两版口径（疑似部分快支秽盾基准实为 200t 致时间口径差一倍；喧响 27.5/s 校准仅对未特调角色成立）；仪玄强特六条维持不展示不评估，耗能口径已确认与 catalog 一致（凝云术 2s 通道共 40、墨痕化形 #1=40/#4=20、墨烬影消=20）。已定案：轻/重招架标准常数 = 数据校准值 **95.511+130t / 95.178+130t**；般岳「冲霄」= 闪避反击公式 + 时间 −1.5s；真斗「孤影·断獠」= 弹刀后连续攻击、改判闪避反击（纯 t 列喧响/积蓄精确命中），真斗无支援突击锚点、直伤记「—」；露西两段强特各耗 60 各自成式（逐段评估正确），妮可 #1 蓄力 20 能量/s、#2/#3 炮击每次耗 60 并留能量场、#4 能量场——妮可逐段无单一标准式，明细仅供参考。
