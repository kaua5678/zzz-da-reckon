...
3. **每期新角色 · 强队强度卡**（页面底部，横轴 = **版本卡池期**，与上面的危局期数轴不同）：每个点 = 当期新 S 角色的**强队**（用户指定清单，预填 `src/data/strongTeamPresets.ts` 用户口述预设（2026-08-23，18 队，如 2.0 仪青潘 / 2.4 仪琉卢 / 3.1 爱芮维琳娜蕾米），仓库 preset 补剩余；每行可手改三人或点「引擎建议」从 `STRONG_TEAM_SUGGESTION_POOL` 补一个候选）。强度按**当前全部已实装**（队伍可含晚于主C实装的队友，如 诺姆 队里的 佩洛伊斯 是 3.2 测试服角色）+ 所选金数配装（默认轻量档主C优先确定性分配；勾选最优加金则逐金贪婪）。未配置强队的角色不出点；3.2 测试服行带标注。
...
| 每期新角色·强队强度（Chart 3） | `teamTimeline.ts`：`buildNewCharacterRows`（版本×新角色行）/ `suggestStrongTeam`（引擎建议，`STRONG_TEAM_SUGGESTION_POOL`）/ `computeNewCharacterPoints`（用户清单逐队配装+收敛过滤）/ `prefillStrongTeamsFromPresets`（口述预设优先 + 仓库 preset 补剩余）；**口述强队单一事实源 = `data/strongTeamPresets.ts`** |
...
- **待确认口径（页面底部有同款清单）**：个别减半/离群的快速支援记录（安东、卢西娅等，按标准版评估由偏差清单呈现）；普攻回能显式录 0 的 15 条以「0% + 打标」展示、不参与聚合（爱丽丝「星芒圆舞曲」三段经 **gachabase 双源确认官方即 0**，柚叶/席德/克拉蕾/洛克茜的待第二源复核，爬取脚本 `scripts/fetch-gachabase-agent.mjs <agentId> <slug>` 已备好，输出 `data/raw/gachabase/`）。已定案：**快速支援两版口径**——标准版 = 伤害 200t（原表 100t 系笔误）+ 原表其余列；翻倍版 = 录制时间只有一半（秽盾基准 50t），评估按 `MOVE_TYPE_TIME_SCALE` ×2 还原、伤害 100t、其余列同标准版；分版判据 = 喧响速率 ≥40/s。**倍率行融合 `MOVE_FUSION_GROUPS`**：星见雅「飞雪 #1+#2=斩击 / #3+#4=追击」（融合后=nanoka 官方倍率，代入公式 ≈1.001）、连携「春临 #1~#3」三段合一（≈1.000）、仪玄「墨痕化形 #1+#2 / #3+#4」——前缀项/耗能只计一次。仪玄强特恢复展示（耗能与 catalog 一致：凝云术 2s 通道共 40、墨痕化形 #1=40/#4=20、墨烬影消=20），特调严重由偏差如实呈现。另有定案：轻/重招架标准常数 = 数据校准值 **95.511+130t / 95.178+130t**；般岳「冲霄」= 闪避反击公式 + 时间 −1.5s；真斗「孤影·断獠」= 改判闪避反击，直伤记「—」；露西两段强特各耗 60 各自成式；妮可 #1 蓄力 20 能量/s、#2/#3 炮击每次耗 60 并留能量场、#4 能量场——明细仅供参考。
