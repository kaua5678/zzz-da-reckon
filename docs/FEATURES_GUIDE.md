...
3.2. **控制技（紫光技）× 反制支援整组替换**（声明了 `counterAssistGroups` 的 Boss，逐组记该组招架段数）：队内有**反制支援**招式的角色（现全库唯一 = 克拉蕾(1611)）时，Boss 卡「反制支援整组替换」默认勾选 → 一组控制技 = 她**一次反制支援**（本体 + 其专属支援突击合成一次动作），**不产弹刀行、不拿弹刀 215 喧响、不参与每次弹刀失衡反推**；取消勾选（或队内无人）→ 整组按「每组 1 次正常弹刀 + 段数−1 次无突击弹刀」并入 3.1 的强制弹刀总数。承接槽位缺省自动（队内第一个有该招式的角色），可在 Boss 卡改为指定槽位。交互栏在「弹刀次数」标签上标「（控制技 N 组已转反制支援）」。两条附带口径（用户 2026-09-12）：**角力 = 一次弹刀同等难度**（`INTERACTION_WEIGHTS.counterAssist = 1.0`，难度明细以「反制支援（角力）N×1」露出，并按承接槽位的截断存活率缩）；**每组直接送 1 层残痕**（琢形原文「重击命中直接添加1层」→ 进克拉蕾残痕/毁伤账本，不吃积蓄效率倍率）。实现：折算 `src/stores/config.ts#syncBossInteractionPlan`（不改编排层）、产行 `src/core/resource/helpers#buildExecutions`、招式配对表 `src/data/counterAssists.ts`；口径见 `ENGINE_PIPELINE_GUIDE.md` §4 坑 18 末段。**当前无 Boss 录该字段**（通道已建，数据待逐 Boss 手录）。
4. 底部「全部 Boss」折叠：23 个 Boss 按「危局异构（困难）」「危局常规（普通）」两组分组，点 chip 跳转到该 Boss 最新危局期。
...
| 要改什么 | 改哪里 |
| --- | --- |
| 预设收录哪些 Boss | `CATALOG_MONSTER_MAP`（**危局 Boss**：困难异构 + 普通常规，共 22 个（含 3.3 测试服临时预设 40012）+ 用户确认收录的老防卫战 Boss 彷徨猎手 `30041`；`null` = 无 catalog 条目） |
...
1. 「时间图表」Tab → 选主C（只列 S 级，默认仪玄）→ 选 Boss（**必选直选**，按最近出场倒序，默认最新危局 Boss；数值取该 Boss 最新一期，优先危局）→ 编辑**候选队友策展池**（多选，localStorage 持久化；默认种子 = 青衣/潘引壶/橘福福/卢西娅/琉音，至少 2 名）→ 限定金预算 → 计算。
   ...
   - **多队并存强度卡**（队伍×期数矩阵，参照用户 演示.xlsx）：池内每个收敛组合一条横带（纵轴=伤害/血量%），覆盖其存活的期数格；每期包容前 K 名（K 可调，默认 3），跌出即永久淘汰（可达集合只增 ⇒ 排名单调不升）并标 ✕。悬停看伤害与存活区间。
     ⚠ 实现坑（一行）：逐期排名「可达集合为空」不能 break——首期可能无任何可达组合（新队友未实装/全被收敛排除），提前退出令后续所有期裁剪失效（曾致弱队全部存活到最后）；实现与修复见 `src/composables/teamTimeline.ts`。
2. 输出：
...
4. **同槽位角色对比卡**（用户 2026-09-04）：预设里大量队伍主C与支援相同、只有击破位不同（如 仪玄+卢西娅 下 琉音 vs 诺姆）。选**对比槽位**（主C/击破/支援，默认击破）+ 两名角色后点「对比」：从 `data/teamPresets` 找「其余两槽完全相同、所选槽位恰好一队 A 一队 B」的成对队伍（相同队伍三元组去重——手编/auto 重复），每组按所选金数配装求 A/B 两队的伤害。**横轴 = 主C实装节点（卡池时间轴）**，一个主C对应 A/B 两个点，A 队各点连成一条线、B 队各点连成一条线——两条线孰高孰低即该主C下谁更强；同主C多支援组合横向错开（同 Chart 3）。**纵轴 = 伤害自动刻度**（贴合 A/B 数据范围 ±8% 边距，不看 Boss 血量/击杀线，只看相对强弱，用户 2026-09-04）。**卡片右上角可单独切 Boss**（默认跟随顶部；手动改过后不再跟随）——不同 Boss 的弱点/抗性对不同角色克制不同，换 Boss 重对比看胜负翻转。配装/金数口径与 Chart 3 相同（轻量档主C优先确定性分配；勾选最优加金则逐金贪婪），交互用角色默认基准，两队的差距即所选槽位角色的净贡献。任一队未收敛（maxIter）整组跳过；主C未收录实装节点的组跳过。悬停看两队伤害与**相对差值（(A−B)/B）**，下方汇总表直读「哪些队伍 A > B」。数据层 = `teamTimeline.ts` `findSlotComparePairs`（纯函数）+ `computeSlotComparePoints`（快照/恢复，同 Chart 3）。
5. **菲林经济模拟卡**（页面底部，横轴 = 所选 Boss 登场的危局期数 + 日期）：**主C 固定（顶部选择）+ 队友 = 顶部候选池按当前总限定金自动换最优双人组**（如 金数够时 琉音换青衣、卢西娅换潘引壶；**选的是主C不是队伍**）→ 逐期发菲林（默认 ≈1金/版本 = 15000，可编辑）→ 按**消耗占比**花/存（如给 1 金用半金 = 0.5）→ 抽卡资金按当前最优队的**主C优先**步买金（命座金 = 93.75 抽 = 15000 菲林；音擎金 = 62.5 抽 = 10000 菲林——依据游戏内调频详情：角色池综率 1.6% × 50/50 保底、音擎池综率 2% × 75/25 保底）→ **每期只用「当前期数」的 Boss 血量 + 关卡固有 buff（layer_buff，期视图有数据才应用）+ 队伍**算伤害/血量%。**起点 = 主C 首次 UP 之后的 Boss 初登场**（主C 实装前的期不计）。可选**目标卡池**（把银行菲林一期清空投入抽卡）、**每版本充值预算（元）按性价比固定分配**：月卡（30元→3300菲林）→ 大月卡（68元→≈2600菲林）→ 直充（10菲林/元），**汇率固定不可改**（`data/filmEconomy.ts` `allocateTopUpFilm`）。初始金数/占比/预算均可设定；纵轴 = 血量%（主线）+ 金数（右轴虚线副线），悬停看详情。
6. **抽卡价值 · 危局兑现卡**（页面底部，点「计算」懒加载实战归档）：回答「这张卡抽了以来在危局**实际兑现了多少分**、哪些卡价值高/低」。横轴 = 归档覆盖的危局房间（期 × 关卡，含普通与困难），每行一张卡、气泡 = 当期**边际兑现**（同房间同一玩家「带该卡最佳分 − 不带最佳分」跨玩家取中位数），行末柱 = **累计兑现总和**（实装以来 Σ 边际）；点击行展开逐期明细。分级 T0~T3 = 限定池累计四分位。详见 §4.4。
7. 可选「包含测试服角色（3.2 未实装）」（缺省关，防测试服数值污染曲线；开时 3.2 节点标注「测试服数据」）。
...
- **搜索排名用「预算感知确定性分配」**（主C优先加金）：排名贴近所选金数，换人时机正确；**最终加金 = 逐金贪婪最优**（影画/音擎本体/精炼每金档全候选取最大提升；非限定槽位也可花 1 金佩戴限定音擎）。函数清单见 §4.3 修改入口。
- **收敛过滤**：失衡外层未收敛（`convergence.outerExit === 'maxIter'`）的队伍伤害虚高（实测青衣系阵容 407%+ vs 收敛 meta 队 105-127%），排除出排名（统计在页头显示）。
- **计算性能口径（实测）**：每次伤害求值 ~30ms、全候选 ~900 对 ≈ 27s ⇒ **默认走策展池轻量档（见 4.1）**；防卡死靠阶段边界 yield（单帧 ~60ms）。两条否决：**贪婪试算区不做 yield**（未还原改动让出会触发 store team watch 重入、扭曲试算，曾致潘引壶测试回归）；**配装不按 agentId 缓存**（队友可用 buff 组随组合变化，缓存使同角色副词条跨队漂移，实测末节点伤害 −13%）。
...
- 交互基准：职业基准 roleInteractionBaseline（支援/防护 0 交互，其余弹刀6/闪反10；般岳/星徽·比利按角色默认）+ 快支3/连携1；配装 = 邦布精灵推荐（含词条优化器）；当期可选 Buff 不参与（沿用当前全局 Buff 表）。
...
**算法**：
- 外层 beam search：状态 = (持有集 × 银行)，每版本节点展开「买卡（窗口内下一档）/ 攒着（实物期权）」，同节点连买深度 ≤3；去重键 = 持有集 + 银行（1000 菲林量化）+ **累计分**（score 必须进键——否则不同质量前缀被合并丢信息，曾致规划器永远不买卡）。选 beam 不选贪心 = niche 协同（卢西娅单拿弱配命破 C 强）破坏次模性、次模常数因子保证不适用（引用见「学术引用锚点」）；实现 `planPullStrategy`（入口见 §4.3）。
...
| 测试 | `src/composables/__tests__/teamTimeline.test.ts`（数据不变量/基础金/贪婪金/演变冒烟/Chart 3 预填/Chart 7 配对与对比求值）；Chart 5 = `pullValue.test.ts`（fixture 不变量 + 真实归档冒烟）+ `pvRenderCheck.test.ts`（渲染层集成） |
...
- **待确认口径（页面底部有同款清单）**：个别减半/离群的快速支援记录（安东、卢西娅等，按标准版评估由偏差清单呈现）；普攻回能显式录 0 的 13 条以「0% + 打标」展示、不参与聚合（爱丽丝「星芒圆舞曲」三段经 **gachabase 双源确认官方即 0**，柚叶/克拉蕾/洛克茜的待第二源复核，爬取脚本 `scripts/fetch-gachabase-agent.mjs <agentId> <slug>` 已备好，输出 `data/raw/gachabase/`）。席德(1461) 错位攻击数据已按 `scripts/patch-misplaced-attack-data.mjs` 回填（通道定义与验证数字见该脚本头注释）。已定案：**快速支援两版口径**——标准版 = 伤害 200t（原表 100t 系笔误）+ 原表其余列；翻倍版 = 录制时间只有一半（秽盾基准 50t），评估按 `MOVE_TYPE_TIME_SCALE` ×2 还原、伤害 100t、其余列同标准版；分版判据 = 喧响速率 ≥40/s。**倍率行融合 `MOVE_FUSION_GROUPS`**：星见雅「飞雪 #1+#2=斩击 / #3+#4=追击」（融合后=nanoka 官方倍率，代入公式 ≈1.001）、连携「春临 #1~#3」三段合一（≈1.000）——前缀项/耗能只计一次。**仪玄强特 = 定点整链记录 `FIXED_RECORD_UNITS`**（用户口径，moveId/耗能/变体定义在 `src/data/standardMultiplierTable.ts`）：墨痕化形按交互变体（单E/2连/3连 × 弹刀与否）整链记录，单E/2连 = 40 闪能、3连 = 60 闪能；原「#1+#2 / #3+#4」融合评估 ~0.46~2.56 假象已弃用；各变体比值（单E 伤害 ~0.82、3连·弹刀 喧响/积蓄 ≈0.99、全变体秽盾 ≈1.000）由演算页偏差清单呈现。**普罗米娅强特 = 定点整链记录**（用户口径）：处刑式·坠霜/重霜 = 消耗 40 能量后接的免费招，记为强特公式 + 40 能量奖励（重霜 伤害 ≈1.023 / 坠霜 ≈0.922 / 秽盾 ≈1.000；按 e=0 特殊技公式会得 2.2~3.1 倍虚高比值、纵向积蓄系数被污染至 125.1%）；重霜 #1+#2 合一，封喉霜径保留原行。另有定案：轻/重招架标准常数 = 数据校准值 **95.511+130t / 95.178+130t**；招式改判走 `MOVE_TYPE_OVERRIDES`（般岳「冲霄」= 闪避反击公式 + 时间 −1.5s、真斗「孤影·断獠」改判闪避反击且直伤记「—」）；露西两段强特各耗 60 各自成式；妮可 #2/#3 炮击每次耗 60——个案明细以演算页偏差清单为准。
...
1. 「实战对比」Tab → 期数 / Boss / 仅击杀 / **仅看低金顶分** 筛选 → 列表点「部署」。
2. 右侧：玩家实战（分数/用时/击杀/队伍/视频链）vs 计算器（总伤害 / 当期 Boss 血量 / 伤害血量% / 击杀判定）。
3. 判定不一致给方向提示：预测击杀但实战未击杀 = 理想配装被高估（配装差 + 建模误差）；实战击杀但未达线 = 口径偏保守（漏拐/漏机制）。

**仅看低金顶分**（`composables/limitedGold.ts` `lowGoldFrontier`）：归档每条 run 有大量高金（满配）噪声投稿，勾选后按房间（期 × Boss）只保留**击杀顶分里限定金数最低**的一批——用最少限定金**实际击杀**该房 Boss 的队 = **角色上限**（计算器算的是角色理论理想值，这些低金击杀顶分队是「该角色/队伍实际能打到的上界」，两者直接对照）。附带「金数窗口」筛选器（最低金 / +1 ~ +4）控制保留几个金档（「每期最低的几金」的「几」）。列表新增「金」列显示限定金数。金数口径同效率前沿：限定 S 本体 1 + 影画 + 专武精炼−1，常驻 S / A 级不计。
...
### 7.6 图表系列筛选（所有多系列图表共用一套口径）

**交互**：图例项可点 = 显隐该系列（该页原本唯一有筛选的血量膨胀图，2026-09-12 起收敛为共享实现，其余图表补齐）。
**唯一实现** = `src/composables/seriesFilter.ts` 的 `useSeriesFilter()`（页面只喂「系列 id 清单」；导出面 = `isVisible` / `toggle` / `showAll` / `filter` / `counts`）；判据 = `__tests__/seriesFilter.test.ts`（9 条）。三条口径（改交互前先读）：

1. **默认全可见，且新系列自动可见**：`hidden` 只存「被用户关掉的 id」，不存白名单——重算/换期数/Boss 后冒出来的新系列默认显示，用户关过的记住。
2. **筛选必须传导到派生量**：隐藏只影响「画什么」，但**轴上限/刻度/明细表/排名表必须一起跟着变**——否则轴按隐藏数据缩放，筛选看起来没生效（各页面在 computed 里读 `filter(...)` / `isVisible(...)`，不要读原始数组）。**唯一例外**：Top-K 存活判定这类**数据性质**（K 是游戏约束，不是显示选项）只跳过绘制。
3. **不允许把最后一个可见系列关掉**（全关 = 空白图，分不清「筛没了」还是「没数据」）⇒「可见数 ≥1」是 API 层不变式，调用方无需再兜。

各页接的维度（都走同一组合式，不是各写一遍）：

| 位置 | 图 | 可筛维度 |
| --- | --- | --- |
| 队伍对比页 | 散点图 | 队伍（图例 chip；隐藏队退出 x/y 轴上限 + 明细表） |
| 队伍对比页 | 难度曲线 | 队伍（隐藏队退出 costMax/ratioMax 轴 + 关键变化 + 曲线摘要） |
| 时间图表页 | 多队并存强度 | 队伍横带（**只少画一条带**：Top-K 排名与淘汰判定是数据性质，隐藏不让别的队递补存活） |
| 时间图表页 | 每期新角色强队强度 | 队伍构成（隐藏队退出 Y 轴刻度 + 悬浮命中） |
| 时间图表页 | 直伤系数散点 | 档位（加强 >105% / 持平 / 削弱 <95%，与散点颜色同义；Y 轴固定 0.7~1.3 档位口径不随筛选缩放） |
| 时间图表页 | 菲林经济模拟 | 队伍强度主线 / 金数副线（隐藏金数线时右轴刻度一并收起） |
| 时间图表页 | 抽卡价值 | 分级 T0~T3 + 样本不足（与「层」下拉正交：层 = 卡池归属，分级 = 兑现强弱） |
| 时间图表页 | 抽卡规划器 | 泳道（购买 / 期分 / 房1~房3；隐藏后留空位不重排，避免开关一下整图跳动） |
| 时间图表页 | 同槽位角色对比 | A/B 两条线（隐藏一队后纵轴按剩下的线缩放） |
| 血量膨胀页 | Boss 版本系数 | Boss（隐藏线的系数退出 Y 轴上限） |
