...
19. **时间预算欠打回填 + 轴退化（2026-08-30）**：坑 12 的折叠循环是单向的（只折正 excess），
    ...
    **连 cfg 一起回滚**（试探轮跑 `iterate` 会触发模块写回：叶瞬光自动选轴在
    ...
    ⑥**截断不回灌资源循环（2026-09-11 用户三问实测；A 项已立项）**：装配期截断只削**行**，而资源账本
    （`calcEnergySource/calcDecibelSource` 走 `materializeRows(state)`）取的是**未截断**的收敛态 ⇒ 有截断时
    资源池总量/次数与 180s 计划不自洽。实测般岳+诺姆+卢西娅（熔狱行赭 3.2 · 全关档）：
    `overflowSeconds == convergence.timeTruncatedSeconds == 66.8s`（21 条行：论道 7×0.70、狮子吼·怒 7×0.70、
    终结技 2×0.67、不动如山 17×0.68…），而物化 `rowsNet` 179.3 ≤ 180（池子只显示「已打满」）；
    槽0 行级回能 账本 200.0 vs 截断后 Σ行 140.0（槽1 39.0 vs 27.3、槽2 14.7 vs 7.3）。
    **合轴率就是周转手段**（实测：合轴抵扣 0→40.0→80.1s 时截断 66.8→25.9→0，且 +100% 后多出 13.8s 平A池、
    2 轮收敛；全关档 5 轮不收敛）。**止血已落**：结果页「时间分配汇总」新增「时间截断 X s + 被砍招式清单」
    （旧文案「超预算」作废——那是把截断误标成"净占用超预算"，而净占用其实 ≤ 预算）、难度轴交互次数按
    `kept/requested` 缩（无截断 = 因子 1，故只动带截断的队）。**正解 = DEBT_REGISTRY 的
    `src/core/resource.ts:截断不回灌资源循环`**（用户给定语义：先按预算重分配平A池、交互只取「达成目标的最少要求」，
    装不下就重收敛，直到截断为 0）。
    **降配/退化判据的既有缺陷与 2026-09-11 修正**：`overBudget` 原只读**截断之后**的净占用
    （`netFrontlineOccupation`），而截断保证它恒 ≤ 预算 ⇒ 对「必要行本来就装不下」的队**永不成立**，
    非轴降配/轴退化等于死代码。修正 = 非轴降配的**触发**加一条臂 `truncatedToo`（装配期真截断
    `overflowSeconds > 1s`）。**生效面实测**：对 119 预设 **0 delta**（预设场景本被净占用臂/轴路径覆盖），
    对**手组默认配置**有效（`仪玄+洛克茜+卢西娅` 截断 10.7→1.6s、降配 ×0.906）；**仍有 19 预设结构性截断**
    （缩交互也装不下）= 轴侧触发 + 逐模块退化的工作面。
    ⚠️ **验收臂决定结果（2026-09-11 复核，纠正「试算不纯」的早前判断）**：把「截断 ≤1s」并进验收会把好试算拒掉，
    于是 `r` 停在**二分前的基线态**——实测 `yixuan-roxy-lucia` 基线（非轴态）超预算 3.78s，接受 scale=0.90625
    后 0.96s（≈棘轮基线 1.0）。**受控实验未发现被拒试算留下可观测副作用**（显式拒绝全部试算 = 基线值；
    关热启动、关本触发臂同样复现同一基线）⇒ 「+2.8s 回归」是**验收太严**，不是污染；别再拿「收紧验收」当修法，
    要做的是**设计验收目标**（消截断 / 时间账不恶化 / 伤害不降的取舍）。另一条待查：该队两次等价调用的
    `axisFallback` 出现 true/false 分叉，疑属「非实数化队落点随初值漂移」的既有性质（多不动点 + 调用顺序）。
    **降配验收目标（2026-09-11 设计，替换原「净占用 ≤ 预算+2s」单臂）**：三条臂、都相对**二分前的基线态**、
    各带 1s 量化容差 —— ① `truncation(trial) ≤ truncation(base)+1s`（不许砍得更多）；② `overBudget(trial) ≤
    overBudget(base)+1s`（不许更超预算）；③ `slack(trial) ≤ slack(base)+1s`（不许把省下的时间变成留白/发呆）。
    取满足三条的最大 scale。**为什么不能单臂收紧**：单臂只看净占用、看不见"还在按比例砍行"；把「截断 ≤1s」
    并进单臂会把好试算一起拒掉（实测拒绝全部试算 ⇒ 停在基线态 3.78s 超预算）。三臂化后**相对棘轮构造上不可能
    变红**（只拦变差），变红的只有 golden 硬字段（有意改进，按规则 10 归因后重生）。
    实测收益（全库只动这 6 队）：截断 `43.21→0`（般岳+触发器）/ `10.77→0` / `7.82→0` / `3.73→0` /
    `27.19→4.30`（1431-1481-1311）；`billy-roxy` 超预算 `1.99→0.07`；伤害 5 升 1 降（+1.5%~+6.8% / −2.2%）。
    **搜索策略（2026-09-11 同一轮内的第二刀，用户裁决「枚举 + 硬约束取最大可行」）**：二分假定"可行域是 scale
    的下闭区间"，而把「截断 ≤1s」并进验收后二分会在 `yixuan-roxy-lucia` 上把好试算全拒（停在基线 3.78s 超预算）；
    "最小截断优先"又会把结构性溢出队压到 scale=0.0625（交互几乎清零）只为少几秒截断（与「交互只取达成目标的
    **最少要求**」相反）。现行 = **SCALES 由大到小（0.875…0.0625）扫，首个「三臂不更差 且 截断 ≤1s」者即采纳**
    （= 最大可行 scale、保留最多交互）；**无人满足 ⇒ 不动**（保基线态、截断如实上报，交给逐模块退化）。
    效果：残留截断 **15 队 → 4 队**（剩的全是叶瞬光 1431 簇：scale 0.125~0.25 仍截断 28~63s，真结构）。
    **残留 4 队的定性（2026-09-11 实测，别再加阶梯档位）**：这 4 队全是叶瞬光(1431) 簇，根因是
    `setting:yeshuguang.formAxis` 的**声明默认 0 = 打满**（`yeshuguang.ts` settings `default: 0`，
    全仓 stores/teamPresets 都没这个键）⇒ 模块自带的 `full→short_pair→short_mie` 自动阶梯被绕过。
    **但把默认改成 auto 不是修法**（实测 `auto-1431-1481-1491`：full 截断 **117.8s** → 灭极短轴 **74.5s** /
    仅灭短轴 **47.1s**，代价是伤害 −8% / **−21%**）——**阶梯确实压时间但压不干净**（剩 47s），
    因为超预算是**队伍级**的（三槽必要行合计超预算，叶瞬光自身 nec 只从 119.2→100.5）。
    正确处置：**如实报「部分兑现」**（`overflowSeconds` + `truncationCuts` 逐行清单 + 难度轴交互按存活率缩），
    真修法在既有债务「全局实数化收敛重构」（轮数与平A池联立求解）与逐模块结构行收口，不在轴阶梯。
    （模块自己那条注释写着"短轴只省时间不省资源、净占用不降"——本机复测是**部分成立**：确有下降但不归零。）
    **已知成本**：结构性溢出队要付满 8 次整轮试算（试过"先用最小候选探一次、失败即跳过"的成本闸门 ——
    **实测会改结果**（同为"没人满足"的两条路径最终态不同）⇒ 不采用；这条同时再次印证「试算次数/顺序会影响落点」）。
    另：`timeGolden` 的 `over` = `max(0,−slack)`（截断后净占用），**不是**装配期截断 `overflowSeconds`。
    **否决记录（2026-09-11 同日，M1 第一版：资源账本的行级收入改读「按 `state.necessaryTime + basicAttackTime`
    截断后的行」）**：想法 = 「折叠环本来就反复跑 iterate，收入按截断后行算就是回灌，且收敛队截断是恒等变换
    ⇒ 零变化」。**实测否决**：`timeGolden` **320 条 delta / ≈40 条目**，而装配期截断（`over`）变化的只有 5 队
    （`yixuan-roxy 0.252→0.006`、`billy-roxy 1.990→0`、`auto-1591-1481-1211 0→1.942`、`billy-qingyi 0→0.046`）
    ——最大 `auto-1431-1481-1341 **−14.0%**`（slot0 `ult 7→6`、slot1 `ex 28→27`/`ult 3→2`）、`auto-1431-1481-1491 −9.3%`。
    根因：**`iterate` 中途的账本 ≠ 最终账本**，按中途小账本截断收入 ⇒ 次数更少 ⇒ 账本更小 = 自锁的低吸引子
    （棘轮偏置），收敛队也照样被改落点。正确形态 = **A 外环**：收入只按**装配期实测的可行量 `kept`** 注入
    （`cfg.rowTimeLimit`，缺省不截断），并以「**`over == 0` 的队 delta 逐位为 0**」当机器判据；
    设计与迁移顺序见 `.claude/task-ledger-calc-core.md`。
    **否决记录 #2（同日，M1 第二版：A 外环 + `cfg.rowTimeLimit` 注入装配期实测 `kept`）**：机制按设计跑通
    （`pass1 = 100 队零变化` ✓ 证明「缺省不截断 = 既有口径不动」），但**有截断的 19 队收敛 0 / 未收敛 19**
    （4 轮跑满，残差 1.57~67.21s：`auto-1431-1481-1491 67.21`、`banyue-liuyin 52.32`、`auto-1431-1481-1341 51.50`、
    `auto-1471-1571-1451 46.20`、`banyue-roxy 45.48` …）。根因：这些队的**必要行是结构性的**（模块按机制推行，
    不受回能支配），折叠环的 `timeBudgetExcess` 再把它们折进账本 ⇒ 缩收入碰不到必要行（`basic → 0` 后
    Σ必要 仍 > 预算）。**结论：A 项剩下的杠杆不是账本而是必要行本身** —— 要么目标层不录取会把旋转推入
    结构性溢出的档，要么模块自退化/判「不可行」；这正是既有 `interactionScale` 降配覆盖不到的一类（坑22⑤）。
    另记一个**测量陷阱**：`timeGolden` 的 `over` = `max(0, −slack)`（**不是**装配期截断 `overflowSeconds`）——
    判 A 的生效面必须直接读 `overflowSeconds`/`truncationBySlot`。
...
33. **折叠环「必要时间超支」的两套机制互为惰性（2026-09-10 阶段2 立项度量 + 三版否决记录）**：
    ...
    **【2026-09-10 晚用户修正：「整数次数」不再是否决理由】** 用户原话：「**动作次数变小数应该是为了恰好
    在180s，这没必要，因为可以调整队友技能的合轴率稍微超时一些。暂时不否决，一切为了开发。**」
    ⇒ ① 本刀（⑤ 撤专属重推）**不再因整数性被否决**，列为**待重验**；② 判据换成「时间账自洽 + 指标」，
    整数性不再是硬门；③ 用户给的机制依据：真实玩家用**队友合轴率**与**轻微超时**吸收，不必为凑 180s 整数化。
    ...
    **【2026-09-10 晚实测复核：这条「真路径」的立项前提已被推翻，动它前必读】**
    `PROBE_CONV_SCAN=1` 现测（127 预设，静态权重）：**`timeBudgetConverged=false` = 0 队**（不是 26/3）；
    折叠轮数分布 `2轮=89 / 3轮=9 / 4轮=2 / 5轮=21 / 8轮=3 / 9轮=1 / 10轮=1 / 21轮=1`；留白 189.3s、超预算 2.2s 不变。
    ⇒ **"非收敛"这个症状已由 `TIME_FOLD_MAX_PASSES = 32`（2026-09-10 早先落地）消化掉**：那些队不是不收敛，
    是**原 8 轮上限不够**（billy-roxy-lucia ρ≈0.70 需要 21 轮）。因此「模块读池产行」的剩余影响只有
    **≥8 轮的那 6 队多跑几轮**（≈10 次额外求值/队），**不是正确性问题**；照原计划去改这 12 个模块的产行，
    会付出全库 golden delta + 保真风险，换来的只是这几队的迭代次数。
    **正确处置**：把 C8 从「改行生成」降级为 ①（可选）**单调性声明 + 机器护栏**（把「允许读池、但必须
    单调递增、禁止拿池做条件跳变」写成契约，防未来新增非单调模块）；② 真要优化就把预算投向
    **那 6 队的残差收敛率**（τ/阻尼方向已被 6 个变体实测否决，见上文）——**须先量 δ 再动**。
