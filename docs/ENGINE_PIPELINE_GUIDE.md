...
33. **折叠环「必要时间超支」的两套机制互为惰性（2026-09-10 阶段2 立项度量 + 三版否决记录）**：
    ...
    **⑥ 停滞判据（2026-09-10，用户裁决后落地，✅ 采纳）**：用户口径「平A 是招式循环的重要组成部分，
    ...
    改为**停滞判据**：连续 3 轮无改善（改善 ≤ 1e-3）即判收敛。实测（127 预设）：
    `timeBudgetConverged=false` **26 → 5 队**（余 3 支比利系 + 2 支 1591 系，残差 0.002~3.1s）、
    ...
    **仍待收口**：3 支比利系（残差 0.4~3.1s，疑与 `runBillyFinalize` 整数重推耦合）+ 2 支 1591 系
    （残差 2~3 毫秒，实际已收敛）。——**根因在模块级反馈**：
