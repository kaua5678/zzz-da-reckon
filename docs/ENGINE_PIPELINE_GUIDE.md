...
19. **时间预算欠打回填 + 轴退化（2026-08-30）**：坑 12 的折叠循环是单向的（只折正 excess），
    ...
    **否决记录（别再重新发明这些，都是量过数字的）**：
    ...
    - 必要前台按可行比例封顶后**回灌平A池** → 留白 391→275s，但**破 `seedInvariance`**（星徽·比利队落点随种子在 23/24 次之间跳）。只写进 `cfg.timeFeasibleScale` 供装配消费，不介入池分配；前置 = 全局实数化收敛（DEBT_REGISTRY）。
...
22. **时间线截断 = 资源循环的硬不变量（2026-09-05 用户口径）**：资源允许的**动作量**超过可用
    ...
    **不要改成"从尾部整行丢"**：装配顺序 ≠ 出招顺序，而模块专属行（叶瞬光架势段、琉音抱拳）
    ...
    叶瞬光自动选轴曾因此被人为关掉）。
    **已知未接**：把「必要前台按可行比例压到装得下」回灌平A池能进一步收留白（实测 391→275s），
    但它改了收敛动力学、**破 `seedInvariance`**（星徽·比利队落点随种子在 23/24 次之间跳）⇒
    前置是坑17 的全局实数化收敛（DEBT_REGISTRY 同名词条）。当前截断在预设库上仅 1 队触发
    （伊德海莉/青衣/卢西娅，代价 1.2s 留白），是**守住不变量的安全网**而非当期收益。
