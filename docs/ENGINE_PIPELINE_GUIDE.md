...
19. **时间预算欠打回填 + 轴退化（2026-08-30）**：坑 12 的折叠循环是单向的（只折正 excess），
    ...
    **否决记录（别再重新发明这些，都是量过数字的）**：
    ...
    - **琉音（1481）强特计划估时钩子单独补（2026-09-06）→ 7 队回归 + 收敛判据破**：三强特（石头/剪刀/布）轮转 × 真实时长 + 送客（转大+终结技）计入必要时间，估时与物化口径一致（无歧义）——但钩子把折叠环残差压到 0.0005s 量级，`maxExcess ≤ 1e-6` 的判据 8 轮耗尽 → `timeBudgetConverged=false`，而 `allAgentsSweep` **硬断言它恒 true**（全含琉音队伍都会红）；且转大赠行的时间守恒破洞随之暴露（见下条）。她的漏计不是孤立可修的口子：要连「折叠收敛判据的实数化」一起动（1051 同款——迭代期实数残差按判稳口径放宽或阻尼），单独补钩子 = 为不成立的收敛判据做手术。
    - **琉音转大赠行的守恒破洞（2026-09-06 定位）**：`applyLiuyinPromote` 声称「转大时间从目标平A池挤出、守恒」，但只抠 `basic_attack` 聚合行——目标平A时间住在**分段行**里时（希格莉德凛冽枪尖/般岳焚身/琉音自己猜拳把戏），聚合行被分段 carve 抠剩 ~0，转大 carve 落空 → 赠行时间全额净增 → 净占用 +7.2s（实测 auto-1591-1481-1311）。修 carve 到分段行（category=basic 全量依次挤出）实测守恒成立、超预算队 8→3，但换 12s 留白 + 般岳/希格莉德 3 个测试钉的旧数字击穿。**正确修法 = 引擎侧预留转大时间（诺姆膛温赠链时间信道同款）**，post-hoc carve 是半修法——留待下一轮与琉音估时一起做。
...
22. **时间线截断 = 资源循环的硬不变量（2026-09-05 用户口径）**：资源允许的**动作量**超过可用
    ...
    ① 平A是填充行先占位，招式行只能用「可用前台 − 平A」；② 次数**必须整数**（复用坑17 的
    「floor + 小数降序预算内加回」装包纪律；等比缩会产出「强特 ×2.78 次」，实测红 11 条）；
