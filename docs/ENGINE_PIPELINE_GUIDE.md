...
35. **留白不是求解器缺陷，是平A池**权重分配**的表征（2026-09-10 实测，重定向整条「留白专项」）**：
    ...
    **落地形态（2026-09-10，用户裁决「做个开关吧，这个算的太慢了，默认关」）**：权重自动分配做成
    **显式开关** `configStore.autoAllocateBasicTime`（**默认关**），策略实现放
    `composables/timeWeightAllocation.ts#TIME_WEIGHT_STRATEGIES`（当前唯一策略=边际均衡）：
    · 关（默认）：静态默认权重 / 用户手填值 —— 即当前两份基线与全部既有数值的口径（**0 delta**）；
    · 开：队伍签名变化时跑一次策略（`CalculatorView` 内 `useTimeWeightAutoAllocation`），
    ...
    · 必须先有生成规则的痕迹：UI 开关在 TeamConfigPage「平A时间权重」旁，tooltip 写明「较慢、会覆盖手改」。
    ...
    **⚠ 增益的形态需要用户裁决（不要照抄 +43%）**：top 增益**全部来自「把弹刀删到 0」**
    （`6/6/0 → 0/0/0`），即搜索判定「弹刀是净时间成本」——因为引擎把弹刀按
    `assistFollowUpActionTime` 记进**必要前台时间**，而其 daze/喧响/闪能回报在小队里低于把这时间
    给主C 的边际伤害。**真实玩法里弹刀是对敌人攻击的反应**（不打也要站着挨着），是否该按「自愿消耗
    前台时间」计价、以及**搜索是否允许减少交互**（还是只允许增加、把预设交互当用户下限），是口径问题，
    须用户定；未定之前该策略只在这个**默认关**的开关后面跑，不参与任何基线。
    **已知限制**：基线已超时的 32 队（含轴模式队）会被直接拒绝优化（`note` 说明），更优处置是
    「先尝试恢复可行性（减交互/挪权重）再谈最大化」——列为下一步候选。
