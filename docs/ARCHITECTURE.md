...
| **引擎内热循环要用到角色专属量**（赠链时间、跨槽回能等——`iterate`/折叠环每 pass 重算，**钩子派发不进去**） | `src/mechanics/types.ts` 的 `crossAgentSupply` 契约 + `docs/ENGINE_PIPELINE_GUIDE.md` §2 | **模块声明能力，引擎按能力查询**：模块写 `crossAgentSupply: { kind, supply(), targetSlot?(), secondsPerUnit?(), decibelPerUnit?(), axisSuppressed? }`；引擎调 `crossAgentSupplyAt`/`crossAgentSuppliesOf`（`core/resource/crossAgentSupply.ts`）与 `findCrossAgentSupplySlots`。**禁止**在 `core/**` 写 `c.agentId === '<id>'` 或 `import ... from '@/mechanics/agents/<角色>'`（两条棘轮盯着：core agentId 16 / core role-import 5） |
...
| **改计算核心的「阶段 / 先后顺序」** | `core/resource.ts#calcTeamResources` 函数头的**阶段表（S0–S5）** 是唯一事实源（每阶段：名字 / 位置 / 输入→输出 / 判据），`core/resource/helpers.ts#iterate` 头注释列 S1 的四步顺序；已抽出的命名阶段 = `runInnerLoop`(S1) / `runFoldLoop`(S2) / `composables/useResourceCalc#stageResolveFeasibility`(S3) | 顺序不可交换（S1 定次数资源 → S2 账本↔行自洽 → S3 撑不下怎么退 → S4 削行 → S5 输出）；改动只落在对应阶段；与 `ENGINE_PIPELINE_GUIDE.md` §1 数据流同源 |
...
| **改难度曲线 / 伤害-难度图型** | `composables/difficultyLadder.ts`（目标集 **G1 权重均衡 / G2 弹刀·联合 / G3 保底 / G4 取整 / G5 合轴率优化（自动·repeatable）** + 逐目标贪心爬梯，纯策略；`opts.base` 可换「全关」基线；`opts.costOf` = 实测操作难度）+ `composables/difficultyCurve.ts`（展示层：散点口径基线 + 纯函数 `buildCurveChart`） | 页面 `views/TeamComparePage.vue` 图型「难度曲线」；口径：x = **自动算的操作难度绝对值**（`computeDifficulty`：Σ 当前档**实打**交互次数×权重 + **时间压力秒×权重**；时间压力 = 硬溢出 `overflowSeconds` + 合轴抵扣 `core/resource/helpers#frontlineOccupationBreakdown().saved`，**同一笔秒数只挂一个权重**〈用户 2026-09-11：「合轴本身就有难度，通过合轴来让溢出时间降低这俩其实是一个东西；允许溢出一部分的原因是队友可以合轴」〉，**与散点页横轴同函数同单位**，故两图可对齐比较；交互次数按**装配期截断存活率**缩到「180s 里真打的次数」〈`rr.convergence.truncationBySlot` 的 kept/requested，用户 2026-09-11：「不上升合轴率导致招式截断，对应的交互次数应该降低」；无截断 = 因子 1，故只有带截断的队会动；曲线按槽缩、散点按团队聚合缩 `teamCompare#shrinkInteractionsByTruncation`〉；权重是主观量、页面「难度权重」弹层可调 + localStorage）、**x 不保证单调**（杠杆减少交互时难度降而伤害升 = 白拿的优化，贪心优先做）、**各队起点/走向不齐是特性**、每档只录取有增益的目标（负收益进 `dropped`）、**配装固定**（缺省 = 该队预设基础金走 `applyGoldSteps`，含 standardSteps；**页面不提供金数档/buff 覆盖**——金数提升属「提升率」类图表，用户 2026-09-10 口径）、不含最优加金/自动下位；页面上还标「多了一次」（Δ≥1 的次数跃迁）+ 每档伤害归因（Σ 分组 ≡ 该档伤害）；判据 `__tests__/difficultyCurve.test.ts` + `difficultyLadder.test.ts`；探针 `PROBE_DIFF_LADDER=1`（跑 UI 同口径，每队约 3~4s）；UI 实机点通 `node scripts/ui-check.mjs --tab 队伍对比 --radio 难度曲线 --main-c --click 计算曲线 --wait-for .curve-seg` |
| **改「选第三人」（固定 2 队友 + 候选范围扫第三槽，不依赖预设队伍）** | `composables/teamTimeline.ts` `computeSlotSweepPoints`（Chart 7 求值口径推广到任意三元组：候选 = `candidateIds` 覆盖，缺省可见角色 − 固定 2 人 `slotSweepCandidates`；单队求值单一事实源 `evalTeamByBudget`——预算感知确定性分配或逐金贪婪，maxIter 未收敛跳过计入 skipped；快照/恢复不留痕；**不含当期 buff/自动下位**——与 Chart 7 同口径；中止粒度 = 一个候选） | 页面 `views/TeamComparePage.vue` 图型「选第三人」（候选圈定 = 职业多选筛选〈空=全部，默认异常〉+ 可再手选子集；伤害降序排名表 + 伤害占比背景条）；判据 `__tests__/slotSweep.test.ts`；UI 实机点通 `node scripts/ui-check.mjs --tab 队伍对比 --radio 选第三人 --click 计算第三人 --wait-for "document.querySelectorAll('.sweep-row').length > 0"` |
...
| 改队伍预设 | `data/teamPresets/*.json`（目录自动加载）；**一级/二级分类口径单源** `scripts/lib/presetCategories.mjs`（一级=输出核心职业队名：强攻/命破/异常/锋御队——击破/支援/防护是辅助位不作分类；二级=该核心属性）；改口径后 `node scripts/sync-preset-categories.mjs` 回填，自动库重生成 `node scripts/gen-auto-presets.mjs`；**同名队只留一条**：`auto-`（实战顶分归档生成）为唯一来源，2026-09-11 删掉 8 条与之重复的手编预设（手编交互/加金顺序是早期死数值，用户裁决「完全不需要以前这个死数值」；其余 19 条无 auto 孪生，保留）；**队伍身份 = 成员集合（顺序无关）**：同 3 人换槽位 = 同队只留一条（用户报障 2026-09-13，此前 `gen-auto-presets.mjs` 去重键用顺序敏感 `join('+')`，产出 11 组重复共 14 条冗余；保留判据 = 金数低 > score 高 > 槽位 0 是输出核心 > run id），生成器同时清理孤儿 `auto-*` 文件；判据 `teamPresets.test.ts`「成员集合去重」 | 同上；`validate:data` 逐条重算分类护栏 + `data/__tests__/teamPresets.test.ts` |
| **实战归档对拍 / 验收读数**（某队打几分、几次失衡、几喧响；改收入口径后复核是否自然达标） | `composables/runArchiveDeploy.ts`（`submissionToDeploy` + `applyDeployConfig` 部署链，参照测试 `archiveDeployStun.test.ts`）+ `core/deadlyAssaultScore.ts`（`scoreForDamageRatio`：单房 65000 = 伤害分 60000 + 操作分 5000）+ `core/resourceTrack.ts`（`simulateDecibelTrack` 窗口时序削大） | 一次性 vitest 探针读 `resourceResult.characters[].decibelSource.total` / `ultimateCount`（琉音赠大看 `source==='gift'` 行，**轴模式下可为小数=期望值，正常**）；**归档只作单条部署对照，不作误差判据、不设基线、不据此拦录入改动**（用户裁决 2026-09） |
...
| **给图表加/改「图例点选筛选」** | `composables/seriesFilter.ts`（`useSeriesFilter()` 唯一实现 + 文件头三条口径：默认全可见 / 筛选必须传导到派生量 / 不允许全关） | 页面只喂系列清单；**轴上限·刻度·明细表·排名必须读过滤后的集合**，否则轴按隐藏数据缩放、筛选看着没生效。判据 `composables/__tests__/seriesFilter.test.ts`；各页接法清单见 `FEATURES_GUIDE.md` §7.6 |
