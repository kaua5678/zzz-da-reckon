...
| **改难度曲线 / 伤害-难度图型** | `composables/difficultyLadder.ts`（目标集 + 逐目标贪心爬梯，纯策略；`opts.base` 可换「全关」基线）+ `composables/difficultyCurve.ts`（展示层：散点口径基线 + 纯函数 `buildCurveChart`） | 页面 `views/TeamComparePage.vue` 图型「难度曲线」；口径：x = **每队自己**的累积难度代价（**各队 x 不对齐是特性**）、每档只录取有增益的目标（负收益进 `dropped`）、不含 buff/加金/自动下位；判据 `__tests__/difficultyCurve.test.ts` + `difficultyLadder.test.ts`；探针 `PROBE_DIFF_LADDER=1`（跑 UI 同口径，每队约 3~4s） |
