...
| **改难度曲线 / 伤害-难度图型** | `composables/difficultyLadder.ts`（目标集 **G1 权重均衡 / G2 弹刀·联合 / G3 保底 / G4 取整 / G5 合轴率优化（自动·repeatable）** + 逐目标贪心爬梯，纯策略；`opts.base` 可换「全关」基线；`opts.costOf` = 实测操作难度）+ `composables/difficultyCurve.ts`（展示层：散点口径基线 + 纯函数 `buildCurveChart`） | 页面 `views/TeamComparePage.vue` 图型「难度曲线」；口径：x = **自动算的操作难度绝对值**（`computeDifficulty`：Σ 当前档**实打**交互次数×权重 + **时间压力秒×权重**；时间压力 = 硬溢出 `overflowSeconds` + 合轴抵扣 `core/resource/helpers#frontlineOccupationBreakdown().saved`，**同一笔秒数只挂一个权重**〈用户 2026-09-11：「合轴本身就有难度，通过合轴来让溢出时间降低这俩其实是一个东西；允许溢出一部分的原因是队友可以合轴」〉，**与散点页横轴同函数同单位**，故两图可对齐比较；权重是主观量、页面「难度权重」弹层可调 + localStorage）、**x 不保证单调**（杠杆减少交互时难度降而伤害升 = 白拿的优化，贪心优先做）、**各队起点/走向不齐是特性**、每档只录取有增益的目标（负收益进 `dropped`）、**配装固定**（缺省 = 该队预设基础金走 `applyGoldSteps`，含 standardSteps；**页面不提供金数档/buff 覆盖**——金数提升属「提升率」类图表，用户 2026-09-10 口径）、不含最优加金/自动下位；页面上还标「多了一次」（Δ≥1 的次数跃迁）+ 每档伤害归因（Σ 分组 ≡ 该档伤害）；判据 `__tests__/difficultyCurve.test.ts` + `difficultyLadder.test.ts`；探针 `PROBE_DIFF_LADDER=1`（跑 UI 同口径，每队约 3~4s）；UI 实机点通 `node scripts/ui-check.mjs --tab 队伍对比 --radio 难度曲线 --main-c --click 计算曲线 --wait-for .curve-seg` |
