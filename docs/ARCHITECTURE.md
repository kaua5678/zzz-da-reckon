...
| **改难度曲线 / 伤害-难度图型** | `composables/difficultyLadder.ts`（目标集 **G1 权重均衡 / G2 弹刀·联合 / G3 保底 / G4 取整 / G5 合轴率优化（自动·repeatable）** + 逐目标贪心爬梯，纯策略；`opts.base` 可换「全关」基线；`opts.costOf` = 实测操作难度）+ `composables/difficultyCurve.ts`（展示层：散点口径基线 + 纯函数 `buildCurveChart`） | 页面 `views/TeamComparePage.vue` 图型「难度曲线」；口径：x = **自动算的操作难度绝对值**（`computeDifficulty`：Σ 当前档**实打**交互次数×权重 + 合轴溢出秒×权重 + **队友合轴节省秒×权重**〈`core/resource/helpers#frontlineOccupationBreakdown().saved`，用户口径：合轴节约出来的时间越多=难度越高、总伤越多〉，**与散点页横轴同函数同单位**，故两图可对齐比较；权重是主观量、页面「难度权重」弹层可调 + localStorage）、**x 不保证单调**（杠杆减少交互时难度降而伤害升 = 白拿的优化，贪心优先做）、**各队起点/走向不齐是特性**、每档只录取有增益的目标（负收益进 `dropped`）、金档可选（走 `applyGoldSteps`，缺省 = 该队基础金，越界钳制）、**当期 buff 可选**（手动选牌，`opts.buff`；**不做自动推荐**——曲线每队本来就要跑 ~10 次求值）、不含最优加金/自动下位；页面上还标「多了一次」（Δ≥1 的次数跃迁）+ 每档伤害归因（Σ 分组 ≡ 该档伤害）；判据 `__tests__/difficultyCurve.test.ts` + `difficultyLadder.test.ts`；探针 `PROBE_DIFF_LADDER=1`（跑 UI 同口径，每队约 3~4s）；UI 实机点通 `node scripts/ui-check.mjs --tab 队伍对比 --radio 难度曲线 --select 曲线金档 --option "8 金" --main-c --click 计算曲线 --wait-for polyline` |
