...
| 决策「哪些招式该打进失衡轴」（捏轴/录轴自动判据） | `composables/__tests__/axisSuggestProbe.test.ts`（`PROBE_AXISSUGGEST=1`；头部注释 = 规则骨架）——按伤害池每招式占比排「窗内价值」，已对 11 个预设轴 + 7 个未收录角色验证 | 规则：候选=直伤行 + combos 轴内块；价值=招式占角色直伤池比（降序）；排除=异常池行（紊乱/异放/DoT，非轴编排项）/generic 平A汇总（有专项块时替代）/占比<2% 小行/单次时长≥窗口40% 长动作；谁进轴=队内直伤占比高者（主C 必进；高伤害击破如琉音也进；工具人不进按覆盖率折算）；单窗容量 stunTime+4+延时 |
