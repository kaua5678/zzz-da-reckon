...
| 改队伍预设 | `data/teamPresets/*.json`（目录自动加载）；**一级/二级分类口径单源** `scripts/lib/presetCategories.mjs`（一级=输出核心职业队名：强攻/命破/异常/锋御队——击破/支援/防护是辅助位不作分类；二级=该核心属性）；改口径后 `node scripts/sync-preset-categories.mjs` 回填，自动库重生成 `node scripts/gen-auto-presets.mjs` | 同上；`validate:data` 逐条重算分类护栏 + `data/__tests__/teamPresets.test.ts` |
