...
⚠ **「数据色」与「自带底色的贴片」是两件事，别混**（2026-09-18 round 31-a2 实测踩到）：

- **数据色** = 色值本身**编码信息**（属性色板、阵容色、Z 值色阶图例）。它**必须**明暗通吃、
  **不跟随主题**——跟随了反而让「图例颜色」与「实际画的颜色」分叉。
- **贴片**（chip / 表头 / inset 行）的**底与墨**是**表面**，不是数据 ⇒ **必须成对跟随主题**。
  实测：写死 `background: rgba(99,226,183,.08); color: #63e2b7` 这类（值抄自 `:root`）在
  亮色档只有 **1.03~1.82:1**。
- ★ **为什么以前没人发现**：判据 5（contrast）的背景**固定取 `--app-panel`**，而贴片是在
  页面底上**又叠了一层** ⇒ 判据 5 对它**结构不可见**（实测那 66 条规则下它 10/10 全绿）。
  已补两条**成对**判据覆盖这一类：行为面 `tinted-contrast` + 形状面 `tinted-ink-closure`
  （跑 `npm run check-tokens` 看，口径注释在 `scripts/check-tokens.mjs` 判据 11/12 处）。
- 判断口诀：**「这个色值换成别的，信息会不会丢？」** 会丢 = 数据色（写死）；
  不会丢 = 表面色（用令牌，浅底用 `--fg-2`、语义底用 `--c-*-strong`）。
...
- **2026-09-18 round 31-a2 贴片墨**：芯片 / 表头 / inset 行这类**自带浅底**的贴片上，
  墨沿用了按**页面底**调的三级墨（`--wa-350`~`--wa-600`）⇒ 亮色档实测 **1.03~3.82:1**
  （66 条规则，其中 23 条连 3.0 都不到）；另有 4 处**手抄 `:root` 令牌值**当字面量
  （`#63e2b7` 等）⇒ 该规则彻底冻在单主题。修法 = 贴片墨统一走 **`--fg-2`**（次级文字，
  在全部 22 种贴片底上两侧最差 **4.63**）、语义底走 `--c-*-strong`；手抄值改回令牌
  （夜间**逐位不变**，亮色档 1.49→4.63）。新增两条**成对**判据：
  行为面 `tinted-contrast` + 形状面 `tinted-ink-closure`（`check-tokens` 10→12 条）。
  ⚠ **豁免只有一条**：`.rs3d-color-spectrum` 的 Z 值色阶图例是**数据色**，
  必须与 `getZColor()` 数值分段逐位一致 ⇒ 不跟随主题（见 §5 的区分）。
