...
游戏文本表达异放倍率只有两种句式，对应引擎两条通道：

| 句式 | 判别法 | 引擎通道 | 先例 |
|---|---|---|---|
| 直接说倍率：「造成相当于X%攻击力的异放伤害」 | 文本直接给数值 | eventType='release' + fields 写 `releaseMultiplier=X`（Type A 固定倍率） | 薇琳娜 145%/255% |
| 拿属性异常伤害说比例：「相对于原属性异常伤害的比例为 A/B/C…」 | **收敛性检验**：各属性 DOT 单发基准（以太62.5/电125/火50/物理713/冰500/风1250，源 `ANOMALY_SINGLE_HIT_MULTIPLIER`）×各自比例，若全部收敛到同一数值 ≈ 固定倍率表达法的伪装 → 按 Type A 固定倍率录（如南宫羽 720/360/900/63/90/36 全收敛到 450%）；不收敛才是真·按原异常伤害结算 | 同上 Type A；真正按原异常伤害结算的场景暂无先例，出现时需新通道 | 南宫羽颤音异放 450%（满层×2=900%，C2 每层35% → 1080%） |

注意：层数/增益系数对全元素同乘——固定倍率在乘系数前就统一了，这是收敛性检验成立的原因。

**异放结算双面板语义（pushRelease → calcAnomalyDamage，全角色通用）**
...
- 60 角色 228 条模式条目实测：L0+L1 合计 47%（不凹分即可全自动正确录入）；L2 近似 43%（AI 建模 + 用户调滑块）；L3 凹分 10%（唯一必须人肉拍板，§4 提炼路径主攻对象）。
- count_cycle 维度里 L3 占比最高（7/25）——「循环次数/资源分配」是凹分理解最密集的地方。
- 逐维度明细表已删（快照会随角色增加过期）；要重测按 §2 各维条目重新归类计数。
...
（2026-09-14 清零：22 条原始条目逐条 grep 核实已全部落地——实现在 `src/mechanics/agents/*.ts` + spec notes
（如 1051 低血增伤 yidhari.ts、1241 失衡增伤 zhuYuan.ts:66、1451 梦境值计划 luciaElowen.ts、1371 强特链 yixuan.ts），
未建模项已按 SOP §6.10 记入 `MECHANICS_IMPLEMENTATION.md` 各档案段「明确未建模」行（如 1591 破阵段数状态机、
1491 猫凝视触发）。本表属录入期工作台账不是知识，删表不删实现；口径疑问查 `node scripts/zc.mjs facts agent:<id>`
或对应角色档案段。）
...
## 7. 反模式（症状 → 权威坑条目；根因/判据/否决记录以坑表为唯一事实源，此处不复述）

| 反模式 | 症状 | 去处（唯一事实源） |
|---|---|---|
| 招式限定 buff 挂全局 | 伤害虚高（作用到不该作用的招式） | `ENGINE_PIPELINE_GUIDE.md` §4 坑11 + D3 moveId 级 patchExecutions |
| 覆盖率滑块经 panel 走私 | 滑块静默失效 | `ENGINE_PIPELINE_GUIDE.md` §4 坑1 + `AGENT_RECORDING_SOP.md` §3.5「面板 buff 施加点错误」行 |
| 附伤用真实 moveId | daze/anomaly 双计 | `ENGINE_PIPELINE_GUIDE.md` §4 坑5 + D8 假 moveId |
| CD 次数忘记 × 覆盖率 | 回能/次数虚高 | `AGENT_RECORDING_SOP.md` §3.5 覆盖率/占比根因行 + D5 floor + 滑块 |
| 转模重复声明 | 基底双计 | `ENGINE_PIPELINE_GUIDE.md` §4 坑7（贯穿力基底复述即双计） |
| 队伍联动写进编排层 | useResourceCalc 大量 agentId 分支 | `ENGINE_PIPELINE_GUIDE.md` §4 坑13 + AGENTS 规则 6 + D7 applyTeamConfig |
