...
> 状态：v0.1 骨架 —— 分类学与四级确定性模型已定，数据基于 60 角色中 50 个的实测提炼；§3 统计与 §5 候选表会随剩余 10 角色补齐。
...
| **L2 approximation** | "覆盖率/占比/默认次数"必须近似：buff 覆盖、状态常驻、命中频率 | AI 建模 + **可调滑块暴露假设** | 「改滑块 → 结果确实变」生效测试 | ~41% |
| **L3 user_meta** | 凹分理解：轴设计、最优资源分配、上限操作逻辑 | **用户拍板**（[已确认] 或 [猜测·]） | verifications 固化 golden 值 + 命座提升率自检 | ~11% |
...
（实测：188 条模式条目 / 50 个角色已提炼，另 10 个角色提炼中）

| 维度 | L0 直读 | L1 直译 | L2 近似 | L3 凹分 | 合计 |
|---|---|---|---|---|---|
| panel_effect | 0 | 16 | 19 | 2 | 37 |
| resource_pool | 0 | 15 | 17 | 4 | 36 |
| team_link | 0 | 16 | 9 | 1 | 26 |
| multiplier | 0 | 13 | 10 | 2 | 25 |
| damage_event | 2 | 10 | 10 | 1 | 23 |
| count_cycle | 0 | 4 | 10 | 6 | 20 |
| stun_axis | 0 | 6 | 3 | 3 | 12 |
| time_window | 0 | 2 | 6 | 1 | 9 |
| 合计 | 2 | 82 | 84 | 20 | 188 |

**读表**：L1 直译(82)+L0 直读(2) 合计 45% —— 这些就是你说的「不凹分也能全自动正确录入」的部分；L2 近似(84, 45%) 是「AI 建模 + 用户调滑块」的协作区；L3 凹分(20, 11%) 是当前唯一必须人肉拍板的，也正是 §4 提炼路径的主攻对象。count_cycle 维度里 L3 占比最高(6/20)，说明「循环次数/资源分配」是凹分理解最密集的地方。
